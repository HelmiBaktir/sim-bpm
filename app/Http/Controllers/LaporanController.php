...
class LaporanController extends Controller
{
    ...
    public function printLaporanImunisasi(Request $request)
    {
        ...
        $laporan = DB::table("transaksi")->where('tanggal','>=', $tanggal_bawah." 00:00:00")->where('tanggal','<=', $tanggal_atas." 23:59:59")->get();
        $pasien_bayi = DB::table("pasien_bayi")->join('layanan_imunisasi', 'layanan_imunisasi.id_pasien_bayi','=', 'pasien_bayi.id')->where('status_hapus','=','0')->get();
        $puskesmas = DB::table("kepala_puskesmas")->where('status_aktif','=', 1)->get();
        ...
        $imunisasi_jenis_layanan = DB::select("SELECT li.id_pasien_bayi, ijl.* FROM  layanan_imunisasi li, imunisasi_jenis_layanan ijl WHERE li.id = ijl.id_layanan_imunisasi AND ijl.tanggal>='".$tanggal_bawah." 00:00:00' AND ijl.tanggal<='".$tanggal_atas." 23:59:59'");
        ...
        $dompdf = new Dompdf();
        $dompdf->loadHtml(View::make('laporan.print.laporan_imunisasi', 
        [
            'judul' => $judul,
    ...
    public function exportLaporanKunjunganKBe(Request $request) {
        ...
        $laporan = DB::select("
            ...
            LEFT JOIN pasien_dewasa pd ON pd.`no_regis` = lk.`no_regis_pasien_dewasa`
            LEFT JOIN suami_pasien_dewasa spd ON spd.`no_regis_pasien_dewasa`=pd.`no_regis`
